...
import 'package:canvas/old_src/editor/control.dart';
import 'package:canvas/old_src/editor/extra.dart';
import 'package:canvas/old_src/editor/util.dart';
...
class FlexLayout extends CanvasLayout {
  ...
  CanvasLayoutResult performLayout(
    ...
    // Optimized: solve flex sizes in-place without cassowary or allocations
    ...
    while (needsAnotherPass) {
      ...
      double flexUnit = totalFlex > 0 ? remainingSpace / totalFlex : 0;
      ...
      while (child != null) {
        if (_shouldLayout(state, child)) {
          var layoutData = child.item.layoutData;
          var parentData = child.parentData as CanvasFlexParentData;
          if (layoutData is FlexLayoutData && parentData.mainSize == 0) {
            double proposedSize = layoutData.flex * flexUnit;
            if (proposedSize < layoutData.min) {
              parentData.mainSize = layoutData.min;
              remainingSpace -= layoutData.min;
              totalFlex -= layoutData.flex;
              needsAnotherPass = true;
            } else if (proposedSize > layoutData.max) {
              parentData.mainSize = layoutData.max;
              remainingSpace -= layoutData.max;
              totalFlex -= layoutData.flex;
              needsAnotherPass = true;
            } else {
    ...
    while (child != null) {
      ...
      if (layoutData is FixedLayoutData) {
        ...
        if (mainChildSize.isInfinite) {
          ...
          parentData.cachedCrossSize = crossChildSize;
          mainChildSize = parentData.mainSize;
          var childSize = _createSize(mainChildSize, crossChildSize);
          childSize = computeDirectionalFittingSize(childSize, layoutData,
        ...
        var mainChildSize = parentData.mainSize;
    ...
    while (child != null) {
      ...
      var parentData = child.parentData as CanvasFlexParentData;
      if (layoutData is FixedLayoutData) {
        usedMainSize += _getMain(child.size.possibleBoundingBoxSize) + gap;
      } else if (layoutData is FlexLayoutData) {
        usedMainSize += parentData.mainSize + gap;
      }
      child = _resolveNextChild(child, textDirection);
    }
  ...
  void handleDrag(CanvasObjectState parent, CanvasItemState dragged,
      EditorControlDelta delta) {
    if (dragged.item.layoutData is AbsoluteLayoutData) {
      return;
    }
