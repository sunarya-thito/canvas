...
import 'package:canvas/old_src/editor/actions.dart';
import 'package:canvas/old_src/editor/ruler.dart';
import 'package:canvas/old_src/editor/selection/widgets.dart';
import 'package:canvas/old_src/external/widgets.dart';
import 'package:canvas/old_src/editor/scrollable.dart';
...
class CanvasEditorState extends State<CanvasEditor>
    with TickerProviderStateMixin, CanvasEditor {
  ...
  void onPointerScroll(PointerScrollEvent event, CanvasEditor editor) {
  ...
  @override
  CanvasEditorController get controller => widget.controller;
                                    ...
                                    (PanGestureRecognizer instance) {
                                      instance
                                        ..onStart = (details) {
                                          ...
                                          _activeMouseGesture ??=
                                              createMouseGesture();
                                          _dragStart = globalToLocal(
                                              details.localPosition);
                                          _activeMouseGesture
                                              ?.onDragStart(_dragStart!);
                                        }
                                        ..onUpdate = (details) {
                                          var localPosition = globalToLocal(
                                              details.localPosition);
                                          if (widget.gesture.allowSnapping) {
                                            ...
                                            _activeSnapAnchor.value =
                                                snapResult;
                                            if (snapResult != null) {
                                              localPosition -=
                                                  snapResult.snapDelta;
                                            }
                                          } else {
                                            _activeSnapAnchor.value = null;
                                          }
    ...
    for (var entry in result.path) {
      if (entry is CanvasPathHitTestEntry) {
        ...
        bool hasAnyOverlapChildren = item is CanvasObjectState &&
            item.children
                .any((child) => findHitTestResult(child) != PathOverlap.none);
